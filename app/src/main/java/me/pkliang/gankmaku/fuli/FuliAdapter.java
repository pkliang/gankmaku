...
public class FuliAdapter extends BaseAdapter<FuliAdapter.ViewHolder> implements ListPreloader.PreloadModelProvider<Entry> {
    ...
    public void onBindViewHolder(ViewHolder vh, int i) {
        Glide.with(mContext)
             .load(mResponse.getResults().get(i).getUrl())
                .bitmapTransform(new CenterCrop(mPool), new RoundedCornersTransformation(mPool, 8, 0))
                //.crossFade()
                .listener(
    ...
    @Override
    public List<Entry> getPreloadItems(int position) {
        return getData().getResults().subList(position, position + 1);
    }

    @Override
    public GenericRequestBuilder getPreloadRequestBuilder(Entry item) {
